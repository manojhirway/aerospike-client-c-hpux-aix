...
CC_FLAGS = -std=gnu99 -ggdb -fPIC -O$(O)-DMARCH_$(ARCH)
ifeq ($(OS),HP-UX)
CC_FLAGS += -mbig-endian -mlp64
else ifeq ($(OS),AIX)
CC_FLAGS += -maix64 -D__PPC__
else
CC_FLAGS += -Wall -march=nocona -rdynamic
endif

CC_FLAGS += -fno-common -fno-strict-aliasing -finline-functions
CC_FLAGS += -DMARCH_$(ARCH) -D_FILE_OFFSET_BITS=64 -D_REENTRANT
CC_FLAGS += -O$(O) -D_GNU_SOURCE $(EXT_CFLAGS)

ifeq ($(OS),Darwin)
  CC_FLAGS += -D_DARWIN_UNLIMITED_SELECT
  LUA_PLATFORM = macosx
else ifneq ($(OS),HP-UX)
#if this not not HP-UX
else
  CC_FLAGS += -shared
  CC_FLAGS += -rdynamic
LUA_PLATFORM = linux
...
LD_FLAGS = $(LDFLAGS) -L. -llua -lpthread -lcrypto -lm -lz -fPIC -maix64
else ifeq ($(OS),Darwin)
