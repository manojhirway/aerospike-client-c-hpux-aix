...
{
	char *my_str = "udf-put:filename=%s;content=%s;content-len=%d;udf-type=%s;";
	...
	command = malloc(strlen(my_str) + strlen(filebase) + strlen(content_base64) + strlen(as_udf_type_str[type]) + sizeof(uint32_t) + 64);
	if (!sprintf(command, my_str, filebase, content_base64, encoded_len, as_udf_type_str[type])) {
#else
	if (! asprintf(&command, my_str,
