	...
	uint32_t timeout_ms, uint16_t n_fields, uint16_t n_bins)
{
	...
	*(uint16_t*)&cmd[28] = cf_swap_to_be16(n_bins);
	...
	memset(&cmd[12],0,sizeof(uint16_t));

	tmp32 = cf_swap_to_be32(generation);
        memcpy(&cmd[14],&tmp32,sizeof(uint32_t));

	tmp32 = cf_swap_to_be32(ttl);
        memcpy(&cmd[18],&tmp32,sizeof(uint32_t));

	tmp32 = cf_swap_to_be32(timeout_ms);
        memcpy(&cmd[22],&tmp32,sizeof(uint32_t));

	tmp16 = cf_swap_to_be16(n_fields);
        memcpy(&cmd[26], &tmp16,sizeof(uint16_t));

	tmp16 = cf_swap_to_be16(n_bins);
        memcpy(&cmd[28],&tmp16,sizeof(uint16_t));
#endif 
...
#else
			memcpy((uint64_t*)p, &v->value, sizeof(uint64_t));
			*p = cf_swap_to_be64(*p); 
#endif
			p += 8;
			len = 8;
...
#else
	uint32_t my_begin = 0;
	my_begin = cf_swap_to_be32(name_len + val_len + 4);
	memcpy((uint32_t*)begin, &my_begin, sizeof(uint32_t));
#endif
	begin += 4;
...
#else
			uint32_t my_cmd = 0;
			my_cmd = cf_swap_to_be32(remaining_ms); 
			memcpy((uint32_t*)(command + 22), &my_cmd, sizeof(uint32_t));
#endif
		}
...
#else
		memcpy(value, (uint64_t *)buf, sizeof(uint64_t));
		*value = cf_swap_from_be64(*value);
#endif
		return 0;
	}
...
#else
		 	uint32_t my_p = 0;
			memcpy(&my_p,(uint32_t*)p,sizeof(uint32_t));
			p += cf_swap_from_be32(my_p) + 4 ; 
#endif
		}
	}
	return p;
...
as_command_parse_success_failure_bins(uint8_t* p, as_error* err, as_msg* msg, as_val** value)
{
	...
	for (uint32_t i = 0; i < msg->n_ops; i++) {
		uint32_t op_size;
#ifndef __hpux
		op_size = cf_swap_from_be32(*(uint32_t*)p);
#else
		memcpy(&op_size,(uint32_t*)p, sizeof(uint32_t));
		op_size = cf_swap_from_be32(op_size);
#endif
		p += 5;
		uint8_t type = *p;
		p += 2;
